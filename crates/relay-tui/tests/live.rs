...
async fn live_sse_graph_refresh_pane_bytes_input_and_resize() {
    ...
    *fake.graph.lock().unwrap() = Fixture::load(fixture_dir("live-7")).unwrap().graph;
    ...
    assert!(
        screen(&rt).contains("[typing · Esc Esc leaves]"),
        "{}",
        screen(&rt)
    );
    // Esc reaches the agent — it is what stops a run in Claude Code and Codex — so a second one
    // straight after is what leaves.
    press(&rt, Key::ESC);
    press(&rt, Key::ESC);
