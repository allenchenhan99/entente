...
```sh
# whole corpus — every session that has children, across every project
node experiments/context-handoff/asis/export-dataset.mjs --discover "C:/Users/User/.claude/projects"

# one session, in more detail
...
Logs are personal and large, so nothing is vendored here; `asis/data/` is gitignored and
regenerated by the exporter.

## Corpus

**64 delegations across 5 sessions in 4 projects** — every delegation on this machine.

Collection needs care: a session has children only if a `subagents/` directory sits beside
it, which is invisible from the parent file's name. A first pass that looked at one session
captured 36 of 64. `--discover` walks the projects root instead, and a structural re-scan
(parsing `tool_use` blocks rather than grepping for a string) confirms **64 real Agent
calls and zero sessions whose transcripts are missing**. Grep alone over-reports here: a
transcript that discusses `"name":"Agent"` matches itself.

| | |
| --- | --- |
| delegations | 64 (41 at spawn_depth 0, 23 at depth 1) |
| briefs the main agents hand-wrote | 63,281 tokens (avg 989) |
| what the children then spent, non-cache-read | 15,880,171 tokens (avg 248,128) |
| child cache reads | 415,741,045 tokens |
| child turns / tool calls / tool errors | 4,655 / 2,544 / 135 |
| child wall-clock | 8.6 hours |
| **brief as a share of delegation cost** | **0.40%** — a child costs **251×** its brief |

Integrity of the export: 64/64 rows joined to a brief, no empty briefs, no zero-turn
children, no missing final messages, no unmatched rows.

The detailed sections below are from the largest single session (`84912a87`, 36
delegations), which is the one with a re-delegation and a compaction boundary in it.
