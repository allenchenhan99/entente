...
## relayd ↔ termd (`RELAY_HOST=relayterm`)

The Rust `termd` (`crates/termd`, protocol in `docs/relay-term-spec.md`) can replace the in-process TypeScript host
above. relayd stays the brain; the `relayterm` `TerminalHost` (`apps/relayd/src/launch/hosts/relayterm.ts`) only
drives termd, and a reverse proxy (`apps/relayd/src/http/pty-proxy.ts`) keeps every client on relayd's routes:

```
relay pane … / TUI / web app ──HTTP /panes*, /metrics · WS /pty/:id──▶ relayd ──same routes, termd token──▶ termd
                             (relayd session token: Bearer / relay.<token>)         (--token, 127.0.0.1:<port>)
```

- **Binary**: `RELAY_TERMD` → `<checkout>/target/release/termd` → `<checkout>/target/debug/termd` → `termd` on
  `PATH`; none found → the host constructor throws (`cargo build -p termd`) and relayd falls back to the fake host.
- **Lifecycle**: `index.ts` calls `host.start()` right after the resume step, so `setNextPane(n)` (one past the
  run's highest recorded `relay:<n>`) becomes `--first-pane n` and casts are never overwritten; after start
  `setNextPane` is a no-op with a warning. termd runs as
  `termd --listen 127.0.0.1:0 --token <termd token> --cast-dir <relayDir>/runs/<run>/casts [--first-pane N]`
  (stdout parsed for `termd listening on <url>`, 10 s timeout; stderr kept in a 20-line ring for error messages).
  `stop()` sends SIGTERM, SIGKILL after 3 s; relayd's `close()` stops it. If termd exits on its own, every
  later host call rejects with `termd exited (code N): <last stderr lines>`.
- **Token**: the termd token is generated by the host (`RelaytermHost.token`) and never leaves relayd; clients
  keep using relayd's session token in front (`sessionGuard` on the proxied routes, `relay.<token>` on the upgrade).
- **Host port**: `spawn` = `POST /panes { name, argv, cwd, env, prompt, task_id, cols, rows }` → `relay:<n>`
  (a `502` rethrows termd's `agent prompt failed: …` message, so the orchestrator's handling is unchanged);
  `focus` = `POST /panes/:id/focus`; `isAlive` = `GET /panes/:id` `.alive` (404 → false); `kill` =
  `POST /panes/:id/kill`; `killAll` kills every alive pane and stops termd.
- **Proxy**: `GET/POST /panes*` and `GET /metrics` are forwarded with streamed bodies and verbatim status + body
  (`/metrics` is termd's, `host: 'relayterm'`, not re-labelled); `GET /panes/:id/cast` streams the bytes.
  `WS /pty/:id`: session-token check → pane probe (`GET /panes/:id`, 404 before the upgrade) → a client
  socket to termd with subprotocol `relay.<termd token>`, frames piped both ways, either side closing closes the
  other. Many clients per pane are just many proxied sockets.
- **Not yet**: resuming a run does not respawn `relayterm` panes (`persist/restore.ts` only respawns `relay`/`fake`
  panes; see HANDOFF_NOTES.md), no termd auto-build, no daemon/attach.
