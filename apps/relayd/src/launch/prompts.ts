...
2. If ANY information that would change the implementation materially is missing or ambiguous (inputs, interface, definition of done, scope), call ${R.respond_to_contract} with decision "needs_clarification" and numbered questions (Q1, Q2, ...). Then loop: call ${R.await_contract} with since_version = the current version; on "pending" call it again; on "revised" re-read the contract with ${R.get_contract} and repeat from step 2; on "canceled" stop. Do NOT create, edit or delete any file while waiting for clarification.
...
4. Work only inside ${spec.cwd} and only within scope.allowed_paths from the contract; never touch other paths and respect non_goals. Call ${R.report_progress} at milestones (accepted, tests written, implementation done, checks green) and ${R.report_blocker} whenever you are stuck, saying what you wait on.
